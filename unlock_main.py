...
def fetch_json(link_json: str):
    response = requests.get(link_json, headers=headers)
...
def get_batteries(link_json: str) -> list:
    try:
        ...
        if json_keys["dataSet"]["type"] == "battery":
            ...
            html_info = requests.get(html_link, params=payload)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(futures):
                    batteries.extend(future.result())
            return batteries
